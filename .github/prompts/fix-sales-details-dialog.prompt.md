---
description: "Fix Sales details dialog render/layout errors in Flutter (overflow, intrinsic, setState during build)"
name: "Fix Sales Details Dialog"
argument-hint: "Paste the error log and reproduction steps"
agent: "agent"
---
You are fixing a Flutter UI runtime error in the Sales invoice details flow.

Use the user-provided argument as the primary bug report (stack trace + reproduction).

Goal:
- Make the Details action in Sales open a stable invoice-details dialog.
- Eliminate runtime layout/build assertions without regressing behavior.

Scope:
- Focus first on lib/features/sales/presentation/sales_page.dart.
...
3. Keep existing actions and behaviors working (selection, shortcuts, return/cancel action, close action).
