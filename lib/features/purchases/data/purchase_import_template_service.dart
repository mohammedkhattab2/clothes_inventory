...
class PurchaseImportTemplateService {
  ...
  Future<String> saveArabicTemplate({required String targetPath}) async {
    ...
    sheet.appendRow(<CellValue>[TextCellValue('قالب استيراد بنود المشتريات')]);
    sheet.appendRow(<CellValue>[
      TextCellValue(
        'القالب مطابق لعناصر إضافة الصنف. للاستيراد في الطلب: سعر الشراء = سعر الوحدة، والكمية المشتراة إلزامية، والباركود اختياري.',
      ),
    ]);
    sheet.appendRow(<CellValue>[]);
    ...
    final columnWidths = <double>[28, 20, 18, 16, 16, 16, 16, 20, 16];
    for (var i = 0; i < columnWidths.length; i++) {
      sheet.setColumnWidth(i, columnWidths[i]);
    }

    for (var col = 0; col < _headers.length; col++) {
      final headerCell = sheet.cell(CellIndex.indexByString(_cellAddress(3, col)));
    ...
    for (var row = 4; row <= 5; row++) {
      ...
      for (var col = 0; col < _headers.length; col++) {
        final dataCell = sheet.cell(CellIndex.indexByString(_cellAddress(row, col)));
